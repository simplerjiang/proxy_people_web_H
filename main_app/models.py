...
class Others_info(models.Model): #用户信息
    user = models.OneToOneField(User)
    balance = models.DecimalField(max_digits=8,decimal_places=2,verbose_name="用户余额",default=0.00)
    ad = models.CharField(verbose_name="代理广告",max_length=30,default= get1_Code())
    TOKEN = models.CharField(verbose_name="API密链",max_length=15,default=get1_Code())
...
class Software(models.Model): #会员版本
    software_id = models.PositiveIntegerField(verbose_name="会员ID", unique=True)
    software_name = models.CharField(verbose_name="会员名称", max_length=30)
    software_version_number = models.CharField(verbose_name="会员版本号", max_length=30,default="V1.0")
    software_each_time = models.PositiveIntegerField(verbose_name="套餐时间（按天计算）",default=720)
    software_cost = models.DecimalField(max_digits=8,decimal_places=2,verbose_name="价格",default=0)
    software_try = models.BooleanField(verbose_name="试用（无需填写，默认即可）",default=False)
    software_try_hours = models.PositiveIntegerField(verbose_name="试用时长（无需填写，默认即可）",default=0)
    def __str__(self):
        return self.software_name+"  ID:"+str(self.software_id)

class Getmoney(models.Model): #提现，没用
...
class Authorization(models.Model): #没用
...
class Time_code(models.Model): #卡密
    software = models.ForeignKey(Software, verbose_name="会员对象")
    time = models.PositiveIntegerField(verbose_name="时长")
    code = models.CharField(verbose_name="卡密",max_length=30,unique=True) #卡密
    cost = models.DecimalField(max_digits=8,decimal_places=2,verbose_name="价格",default=0) #价格
    proxy_man = models.ForeignKey(User,verbose_name="购卡代理",null=True)
    used = models.BooleanField(verbose_name="是否使用过",default=False)
    deal_object = models.ForeignKey(Deal_record,verbose_name="交易订单",null=True)
    begin_time = models.DateTimeField(verbose_name="存卡时间",default=timezone.now)

    def __str__(self):
        return "卡密："+ self.code + "种类：" + self.software.software_name
...
def get_deal_code(num=5): #获取随机TOKEN，通过传入位数。
    while 1:
        ...
        try:
            ...
            return sa
def get_software_code(num=3): #获取随机TOKEN，通过传入位数。
    while 1:
        a = "1234567890"
        sa = []
        for i in range(num):
        ...
        sa = int(sa)
        try:
            Software.objects.get(software_id=sa)
